// Continuation of 1b: Groups both sketches into non-overlapping super-shingles, then hashes each shingle. 
// Finally counts the amount of super-shingles the documents agree on.
...
function nearDuplicates(doc1: string, doc2: string) {
    ...
    const superShinglesDocA = groupSketch(sketchDocA);
    const superShinglesDocB = groupSketch(sketchDocB);

    const hashedSuperShinglesDocA = hashSuperShingles(superShinglesDocA);
    const hashedSuperShinglesDocB = hashSuperShingles(superShinglesDocB);    

    const similarity = hashedSuperShinglesDocA.intersection(hashedSuperShinglesDocB).size;
    //const similarity = jaccardSimilarity(sketchDocA, sketchDocB);
    console.log(`Super-shingles that agree are: ${similarity}`);
}
...
function groupSketch(shingles: Set<bigint>): Set<bigint>[] {
    const shinglesArr = Array.from(shingles);
    const superShingles: Set<bigint>[] = [];

    for (let i = 0; i < shinglesArr.length; i += GROUP_SIZE) {
        const superShingle = new Set<bigint>();
        const shingleSlice = shinglesArr.slice(i, i + GROUP_SIZE);

        shingleSlice.map(slicedShingle => superShingle.add(slicedShingle));

        superShingles.push(superShingle);
    }
    
    return superShingles;
}

function hashSuperShingles(superShingles: Set<bigint>[]): Set<bigint> {
    const hashedSuperShingles = new Set<bigint>();
    
    superShingles.forEach(shingles => {
        let shingleStr = "";
        shingles.forEach(shingle => {
            shingleStr += shingle.toString();
        })

        const hashedShingle = hashShingle(shingleStr, BigInt(0));
        hashedSuperShingles.add(hashedShingle);
    })

    return hashedSuperShingles;
}
